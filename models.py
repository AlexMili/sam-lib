...
def repeat(x):
    return K.reshape(K.repeat(K.batch_flatten(x), config["nb_timestep"]), (config["b_s"], config["nb_timestep"], 512, config["shape_r_gt"], config["shape_c_gt"]))


def repeat_shape(s):
    return (s[0], config["nb_timestep"]) + s[1:]


def upsampling(x):
    return T.nnet.abstract_conv.bilinear_upsampling(input=x, ratio=config["upsampling_factor"], num_input_channels=1, batch_size=config["b_s"])


def upsampling_shape(s):
    return s[:2] + (s[2] * config["upsampling_factor"], s[3] * config["upsampling_factor"])


# KL-Divergence Loss
def kl_divergence(y_true, y_pred):
    max_y_pred = K.repeat_elements(K.expand_dims(K.repeat_elements(K.expand_dims(K.max(K.max(y_pred, axis=2), axis=2)),
                                                                   config["shape_r_out"], axis=-1)), config["shape_c_out"], axis=-1)
    y_pred /= max_y_pred
...
def correlation_coefficient(y_true, y_pred):
    ...
    N = config["shape_r_out"] * config["shape_c_out"]
...
def sam_vgg(x):
    ...
    # Learned Prior (1)
    priors1 = LearningPrior(nb_gaussian=config["nb_gaussian"], init=gaussian_priors_init)(x[1])
    ...
    # Learned Prior (2)
    priors2 = LearningPrior(nb_gaussian=config["nb_gaussian"], init=gaussian_priors_init)(x[1])
